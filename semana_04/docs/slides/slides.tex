...
\begin{frame}{¿Por qué predecir?}
    \begin{columns}
        ...
        \begin{block}{El dato es el nuevo fertilizante}
            \begin{itemize}
                \item \textbf{Descriptivo}: \textquestiondown Cuánto cosechamos? \\ \textit{\small (Análisis histórico)}
                \item \textbf{Predictivo}: \textquestiondown Cuánto cosecharíamos si...? \\ \textit{\small (Modelos de IA)}
            \end{itemize}
        \end{block}
...
\begin{frame}{Descenso del Gradiente}
    \textbf{\textquestiondown Cómo aprende el modelo?}
    \vspace{0.3cm}
    \begin{columns}
        \column{0.6\textwidth}
        \begin{enumerate}
...
\begin{frame}{Conclusiones y Futuro}
    \begin{itemize}
        ...
        \item \textbf{Próxima Semana}: \textquestiondown Qué pasa si sumamos la lluvia y el pH del suelo? (\textbf{Regresión Múltiple}).
    \end{itemize}
    \vfill
    \centering
